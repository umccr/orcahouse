{{
    config(
        indexes=[
            ...
            {'columns': ['library_id', 'external_subject_id'], 'type': 'btree'},
...
with incremental as (
    ...
    from {{ source('ods', 'metadata_manager_historicallibrary') }} l
        join {{ source('ods', 'metadata_manager_subject') }} s on s.orcabus_id = l.subject_orcabus_id
    {% if is_incremental() %}
    where
...
linked as (
    ...
    from {{ ref('hub_library') }} hl
        join {{ ref('link_library_external_subject') }} lnk on lnk.library_hk = hl.library_hk
        join {{ ref('hub_external_subject') }} hes on lnk.external_subject_hk = hes.external_subject_hk
